...
public class Utility {
    ...
    private static final AtomicLong delayInjectionCount = new AtomicLong();
    private static final Set<Long> firstDelayThreads = ConcurrentHashMap.newKeySet();

    // General delay method
    // TODO: Make it configurable amount of delay (currently constant 100ms)
    public static void delay() {
        delayInjectionCount.incrementAndGet();
        try {
            Thread.sleep(100);
    ...
    public static long getDelayInjectionCount() {
        return delayInjectionCount.get();
    }
    ...
    public static void onCall(InterceptionPoint interception) {
        ...
        if (!bugFound && shouldDelay(interception) && isEligibleForFirstDelay()) {
            //System.out.println("Should Delay");
            firstDelayThreads.add(Thread.currentThread().getId());
    ...
    private static boolean isEligibleForFirstDelay() {
        return !"main".equals(Thread.currentThread().getName())
                && !firstDelayThreads.contains(Thread.currentThread().getId());
    }
